				...
				axios
					...
					.then(function(response) {
						...
						for (let i = 0; i < response.data.list.length; i++) {
							...
							let temp =
								Number(response.data.list[i].main.temp) -
								273.15;

							switch (time.getDay()) {
								case 0:
									forecastObj['Sunday']['hours'][
										time.getHours()
									] = {
										temp: temp
									};
						...
						for (let key in forecastObj) {
							if (
								Object.entries(forecastObj[key]['hours'])
									.length === 0
							) {
								continue;
							}
							...
							for (let hour in forecastObj[key]['hours']) {
								hoursArr.push(
									<div
										key={hour}
										className='forecast-card__entry'
									>
										<span className='forecast-card__hour'>
											{hour}:00
										</span>
										<span className='forecast-card__temperature'>
											{Math.round(
												forecastObj[key]['hours'][hour][
													'temp'
												]
											)}
											°C
					...
					axios
						...
						.then(function(response) {
							if (response.data.data.length > 0) {
								that.setState({
									citiesList: response.data.data.map(
										(city, index) => (
											<li
												key={index}
